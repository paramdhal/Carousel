...
;(function ( $, window, document, undefined ) {
	...
	var curr=1,animationComplete=true,cTimer,div,ul,itemLength,liSize,bezierVal;
	
    Carousel.prototype = {
		init: function () {
			div = $(this.element)
			ul = $(div).children(':first-child').children(':first-child');
			var tLi = $(ul).children(), tl = tLi.size();
			ul.prepend(tLi.slice(tl-1).clone())
	              .append(tLi.slice(0,1).clone());
			var li = $(ul).children();
			itemLength = li.size();
			liSize = li.width();
		...
		cAnimate: function(to){
			 if(animationComplete) {
				if(to<=-1){
					ul.css({left:-liSize*(itemLength-2)+"px"});
					curr = itemLength-3;
		...
		carouselAnimation: function(element,position){
			...
			if($.support.transition){
				...
				$(element).filter(':not(:animated)').animate( { left:"-"+position+"px" }, $this.options.speed,$this.options.easing,function(){
					animationComplete=true;
					if($this.options.slid) $this.options.slid();
				});
			}	
	...
	function setAnimation (selector,time,bezier) {
		...
		$(prefixes).each(function () {
			var prefix = this+'transition',aniArgs={};
	    ...
	    return function (n) {
			...
			var ease = "ease",
				easeIn = ease+"In",
			   	...
			   	names = ["Quad","Cubic","Quart","Quint","Sine","Expo","Circ","Back"],easings =[];
